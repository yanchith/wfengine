...
#[derive(Debug, Clone, Copy, PartialEq)]
pub struct FloatSliderSettings<'a> {
    pub readonly: bool,
    pub speed: f32,
    pub min: f32,
    pub max: f32,
    pub display_decimals: u16,
    pub show_text_input_on_double_click: bool,
    pub theme: &'a FloatSliderTheme,
}

impl Default for FloatSliderSettings<'_> {
    #[inline]
    fn default() -> Self {
        Self {
            readonly: false,
            ...
            theme: &Theme::DEFAULT.float_slider,
        }
    }
}

#[inline]
pub fn float_slider(frame: &mut Frame, id: CtrlId, value: &mut f32, label: &str) -> bool {
    do_float_slider(frame, id, slice::from_mut(value), label, FloatSliderSettings::default())
}

#[inline]
pub fn float_slider_ex(
    frame: &mut Frame,
    id: CtrlId,
    value: &mut f32,
    label: &str,
    settings: FloatSliderSettings,
) -> bool {
    do_float_slider(frame, id, slice::from_mut(value), label, settings)
}

#[inline]
pub fn float2_slider(frame: &mut Frame, id: CtrlId, value: &mut [f32; 2], label: &str) -> bool {
    do_float_slider(frame, id, value, label, FloatSliderSettings::default())
}

#[inline]
pub fn float2_slider_ex(
    frame: &mut Frame,
    id: CtrlId,
    value: &mut [f32; 2],
    label: &str,
    ...
    do_float_slider(frame, id, value, label, settings)
}

#[inline]
pub fn float3_slider(frame: &mut Frame, id: CtrlId, value: &mut [f32; 3], label: &str) -> bool {
    do_float_slider(frame, id, value, label, FloatSliderSettings::default())
}

#[inline]
pub fn float3_slider_ex(
    frame: &mut Frame,
    id: CtrlId,
    value: &mut [f32; 3],
    label: &str,
...
#[inline]
pub fn float4_slider(frame: &mut Frame, id: CtrlId, value: &mut [f32; 4], label: &str) -> bool {
    do_float_slider(frame, id, value, label, FloatSliderSettings::default())
}

#[inline]
pub fn float4_slider_ex(
    frame: &mut Frame,
    id: CtrlId,
    value: &mut [f32; 4],
    label: &str,
...
fn do_float_slider(
    ...
    let mut s: InlineString<256> = InlineString::new();
    ...
    for (i, value_mut_slot) in value_mut.iter_mut().enumerate() {
        ...
        let (active, changed_i, double_clicked) = if settings.readonly {
            ...
            if state.show_text_input == 0 {
                ...
                let new_value = f32::clamp(value + delta * settings.speed, settings.min, settings.max);
        ...
        if state.show_text_input > 0 {
            ...
            *frame.ctrl_string_mut() = string;
            ...
            s.clear();
            let _ = write!(s, "{:.1$}", value_mut_slot, usize::from(settings.display_decimals));
        ...
        if confirmed_from_kb {
            ...
            match f32::from_str(string) {
                Ok(mut new_value) => {
                    new_value = f32::clamp(new_value, settings.min, settings.max);
