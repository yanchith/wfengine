...
// TODO(jt): @Cleanup VecString only exists to store strings with an allocator. This should
// eventually be removed, once Rust has String<A>.
#[derive(Debug)]
...
impl<A: Allocator> VecString<A> {
    ...
    pub fn try_splice(&mut self, index: usize, delete: usize, insert: &str) -> Result<(), TextCapacityError> {
        {
            // SAFETY: We preserve the invariant that the data always has to be
