...
// TODO(jt): const fn
pub fn srgb_to_linear(color: u32) -> Vec4 {
...
// TODO(jt): const fn
/// Converts an sRGB color channel into a linear color channel.
///
...
fn srgb_byte_to_linear(val: u8) -> f32 {
    #[rustfmt::skip]
    static D3DX_SRGB_TABLE: [u32; 256] = [
    ...
    f32::from_bits(D3DX_SRGB_TABLE[usize::from(val)])
}

// Taken from D3DX_DXGIFormatConvert.inl, ported to Rust.
#[allow(dead_code)]
fn linear_float_to_srgb(mut val: f32) -> f32 {
    if val < 0.0031308 {
        val *= 12.92;
    } else {
        val = 1.055 * libm::powf(val, 1.0 / 2.4) - 0.055;
    }
